...
from int import lexer
from .lexer import tokens, arregloHtml, contadorErrores
from importlib import reload  

exportarTxt = list()
contadorErroresParser = 0
contadorErroresParser = contadorErrores 

arregloHtml = lexer.arregloHtml
...
def p_error (p):
    ...
    global contadorErroresParser, errLine
    if (p):
        print("Que da p:", p)
        ...
        print(exportarTxt)
        errLine = 0
        exportarTxt.append(['Error parser --> desconocido'])
    contadorErroresParser += 1
...
if __name__ != "__main__":
    def analizarApi(txt):
        global contadorErroresParser, errLine, exportarTxt
        ...
        if contadorErroresParser > 0:
            result = False

        # Exportaciones
        htmlResult = exportarHtml(arregloHtml)
        analisisResult = []
        analisisResult.append('Producciones analizadas por el parser\n-----------------\n')
        contador = 0
        for line in exportarTxt:
            contador += 1
            analisisResult.append(f'{contador}) {line[0]} | {line[1]}\n')
            analisisResult.append('-------------\n')
        analisisResult.append('-------------\n')
        analisisResult.append(f'Total de tokens analizados: {contador}.\n')

        res = {'result': result, 'errline': errLine, 'html': htmlResult, 'analisis': analisisResult}
        reload(lexer) # Reinicia Lexer para evitar guardar estado anterior
